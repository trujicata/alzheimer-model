...
class Tokenizer(nn.Module):
            ...
            *[
                nn.Sequential(
                    nn.Conv3d(
                        ...
                        padding=(frame_kernel_size // 2, padding, padding),
                        bias=conv_bias,
                    ),
                    nn.Identity() if not exists(activation) else activation(),
                    nn.MaxPool3d(
                        kernel_size=(
                            frame_pooling_kernel_size,
                            pooling_kernel_size,
                            pooling_kernel_size,
                        ),
    ...
    def sequence_length(self, n_channels=1, frames=120, height=100, width=100):
        return self.forward(torch.zeros((1, n_channels, frames, height, width))).shape[
            1
        ]
        ...
        **kwargs,
        ...
        if positional_embedding == "none":
            ...
            self.register_buffer(
                "positional_emb", sinusoidal_embedding(sequence_length, embedding_dim)
            )
